...
app.post('/ajouter-pbx', requireAdmin, async (req, res) => {
  try {
    ...
    res.redirect('/liste-pbx');
  } catch (e) {
    console.error('Erreur lors de l\'ajout du PBX:', e);
    res.status(500).send(`
...
app.post('/filtrer', async (req, res) => {
  ...
  `;
  try {
    const [results] = await pool.query(query, [pbx, fromTimestamp, toTimestamp]);
    res.json(results);
  } catch (err) {
...
<body>
  ...
  <form method="get">
    ...
    <div>
      <button type="button" onclick="window.location.href='/liste-cdr'">Réinitialiser</button>
    </div>
  </form>
  <table>
    <thead>
